...
# go through each path one step at a time
...
def graphTraversal(graph, startingRoom):
    ...
    # Pop the first room 
        ...
        if key not in visited:
            ...
            # find the direction that got us to this room, by searching from the prev room
            ...
            if prevRoom != None:
                for direction in old_directions:
                    if old_directions[direction] == key:
                        # put the name of the previous room and the direction you took onto the path
                        path.append({key: direction})
                        isDeadEnd = False
                        break
            if isDeadEnd == True:

                # time to implement breadth first search 
                q.enqueue(v)
                # this means we have a queue we will use, that's separate from the stack
                while q.size() > 0:
                    node = q.dequeue()
                    ...
                    if key in visited:
                        ...
                        directions = list(graph[key].items())[0][1][1]

                        # if so, add all possible children of that key to the queue for future BFT
                        for direction in directions:
                            q.enqueue(directions[direction])
                    # if the key is not visited, we can break the function and start a new depth first search on the next loop

                    else: 
                        s = Stack()
                        start = {key: graph[key]}
                    # once we've hit the closest exit, clear the queue and stack, and start over again 
                        s.push(start)
...
# use bread first search to find the nearest exit 
# once you find the nearest exit, continue using depth first search, until you hit the end. 

# the starting vertex will change depending on when we hit the end point
# that will also change the children that 

# think of these as determining the sequence of instructions you're providing to a computer based on where the
# objects are placed in the hard drive

# the key thing is to be able to switch after one has hit the end of a depth 
# what condition are you looking for? 

# check to see if there's no where else to turn (i.e. other places have been visited)
